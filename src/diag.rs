...
/// Writes one diagnostic line to stderr, stamped with the elapsed milliseconds and the
/// calling thread, when `XODUS_DIAG` is set. Arguments are only evaluated if it is.
macro_rules! diag {
    ($($arg:tt)*) => {
